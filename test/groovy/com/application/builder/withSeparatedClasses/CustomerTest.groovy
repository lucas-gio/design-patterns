...
class CustomerTest extends Specification {
	...
	def "Se verifica que el builder de clientes genera nuevas instancias"(){
		...
		5000.times{
			customerHashCodeList << new CustomerBuilder("1", "a", "b", CustomerType.GOLD).build().hashCode()
		}
	...
	def "Se verifica que el builder de clientes genera correctamente los clientes"(){
		...
		Customer customer = new CustomerBuilder("1", "Julio", "César", CustomerType.GOLD)
							.build()
		...
		customer = new CustomerBuilder("2", "Marco", "Aurelio", CustomerType.VIP)
		...
		customer = new CustomerBuilder("3", "Nerón", "Nerón", CustomerType.SILVER)
				.withBirthDate(new Date().clearTime())
				.withRegistrationDate(new Date().clearTime())
				.withLoyaltyCards([
